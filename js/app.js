...
/* Community Treasury wallet transparency */
const TREASURY_ADDRESS = "6wiJb8mdQSc4dArLByC6c3jZS8MkAtUaMwnvMk2Wh2ES";
const TREASURY_REFRESH_INTERVAL = 30 * 60 * 1000;
let treasuryLastUpdated = null;
let treasuryLoading = false;

const treasuryMessages = {
  en: { loading: "Consulting the Solana blockchain…", success: "Balance verified on the Solana blockchain.", error: "Balance unavailable. You can still verify it directly on Solscan.", copied: "Treasury address copied" },
  pt: { loading: "Consultando a blockchain da Solana…", success: "Saldo verificado na blockchain da Solana.", error: "Saldo indisponível. Você ainda pode verificá-lo diretamente no Solscan.", copied: "Endereço da tesouraria copiado" },
  es: { loading: "Consultando la blockchain de Solana…", success: "Saldo verificado en la blockchain de Solana.", error: "Saldo no disponible. Aún puedes verificarlo directamente en Solscan.", copied: "Dirección de la tesorería copiada" },
  zh: { loading: "正在查询 Solana 区块链…", success: "余额已在 Solana 区块链上验证。", error: "暂时无法获取余额。你仍可直接在 Solscan 上验证。", copied: "金库地址已复制" }
};
...
function formatTreasuryTime(date) {
  if (!date) return "—";
  const locales = { en: "en-US", pt: "pt-BR", es: "es-ES", zh: "zh-CN" };
  return new Intl.DateTimeFormat(locales[getTreasuryLanguage()], {
    dateStyle: "short",
    timeStyle: "medium"
  }).format(date);
}

function updateTreasuryTimeLabel() {
  const time = document.getElementById("treasuryUpdatedAt");
  if (time) time.textContent = formatTreasuryTime(treasuryLastUpdated);
}

async function fetchTreasuryBalance() {
  const balance = document.getElementById("treasuryBalance");
  const status = document.getElementById("treasuryStatus");
  const refreshButton = document.getElementById("refreshTreasury");
  if (!balance || !status || treasuryLoading) return;

  const messages = treasuryMessages[getTreasuryLanguage()];
  treasuryLoading = true;
  status.textContent = messages.loading;
  refreshButton?.setAttribute("disabled", "disabled");

  try {
    const response = await fetch("/api/treasury-balance", { cache: "no-store" });
    if (!response.ok) throw new Error("Treasury API unavailable");
    const data = await response.json();
    const sol = Number(data.sol);
    if (!Number.isFinite(sol)) throw new Error("Invalid treasury balance");

    balance.textContent = `${new Intl.NumberFormat("en-US", { maximumFractionDigits: 6 }).format(sol)} SOL`;
    treasuryLastUpdated = data.updatedAt ? new Date(data.updatedAt) : new Date();
    status.textContent = messages.success;
    updateTreasuryTimeLabel();
  } catch (error) {
    status.textContent = messages.error;
    console.warn("Treasury balance unavailable:", error);
  } finally {
...
document.addEventListener("DOMContentLoaded", () => {
  document.getElementById("refreshTreasury")?.addEventListener("click", fetchTreasuryBalance);
  document.getElementById("copyTreasuryAddress")?.addEventListener("click", () => {
    navigator.clipboard.writeText(TREASURY_ADDRESS)
      .then(() => showToast(treasuryMessages[getTreasuryLanguage()].copied))
      .catch(() => showToast(TREASURY_ADDRESS));
  });

  if (document.getElementById("treasuryBalance")) {
    fetchTreasuryBalance();
    window.setInterval(fetchTreasuryBalance, TREASURY_REFRESH_INTERVAL);
  }
});

window.addEventListener("saintLanguageChanged", () => {
  updateTreasuryTimeLabel();
  const status = document.getElementById("treasuryStatus");
  if (status && treasuryLastUpdated) status.textContent = treasuryMessages[getTreasuryLanguage()].success;
});
